...
class Generator {
    /**
     * Format a given size in bytes to a human-readable format
     * @param {number} size - size in bytes
     * @returns {string} - size formatted as string
     * @example
     * formatFileSize(1024) // returns '1.00 KB'
     * formatFileSize(1024 * 1024) // returns '1.00 MB'
     */
    formatFileSize(size) {
...
class Youtube {
    /**
     * Creates a new instance of the Youtube class
     * @constructor
     */
    constructor() {
     ...
     * @returns {Promise<Object>} - Object containing video information and download link
     * @example
     **/
    async downloadAudio(url) {
        try {
            ...
            const { data } = await axios.post("https://youtubemp4free.com/wp-admin/admin-ajax.php",
                payload,
                { headers }
            );
